                    ...
                    <tr class="text-center align-middle" style="vertical-align:middle;">
                        ...
                        <td>
                            @if($order->status_order === 'pending')
                                <span class="badge bg-warning text-dark px-3 py-2" style="font-size:1em;">CHỜ XÁC NHẬN</span>
                            ...
                            @else
                                <span class="badge bg-secondary px-3 py-2" style="font-size:1em;">{{ strtoupper($order->status_order) }}</span>
                            @endif
                        </td>
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                @if($order->status_order === 'pending')
                                    <form method="POST" action="{{ route('client.orders.cancel', $order->id) }}" onsubmit="return confirm('Bạn chắc chắn muốn hủy đơn này?');">
                                        @csrf
