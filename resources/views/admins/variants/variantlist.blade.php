                ...
                <div class="container-fluid">
                    ...
                    @if ($deletedCount > 0 && request('show_deleted') != '1')
                        <div class="alert alert-info alert-dismissible fade show" role="alert">
                            ...
                            <strong>Thông Báo Thùng Rác:</strong> Bạn có {{ $deletedCount }} biến thể đã xóa trong thùng
                            rác.
                            <a href="{{ route('variants.index', ['show_deleted' => '1']) }}" class="alert-link">Xem thùng
                                rác</a> để khôi phục hoặc xóa vĩnh viễn.
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    @endif
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            <form method="GET" action="{{ route('variants.index') }}" class="row g-3">
                                ...
                                <div class="col-md-2">
                                    <label for="search_price_min" class="form-label">Giá Tối Thiểu (VNĐ)</label>
                                ...
                                <div class="col-md-2">
                                    <label for="search_price_max" class="form-label">Giá Tối Đa (VNĐ)</label>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select class="form-select" id="search_status" name="search_status">
                                        ...
                                        <option value="inactive"
                                            {{ request('search_status') == 'inactive' ? 'selected' : '' }}>Không Hoạt Động
                                        </option>
                                    </select>
                                </div>
                                    ...
                                    <div class="d-flex flex-wrap align-items-center gap-2">
                                        ...
                                        @if (request('search_price_min'))
                                            <span class="badge bg-info-subtle text-info">
                                                Giá tối thiểu:
                                                "{{ number_format(request('search_price_min'), 0, ',', '.') }}₫"
                                        ...
                                        @if (request('search_price_max'))
                                            <span class="badge bg-info-subtle text-info">
                                                Giá tối đa:
                                                "{{ number_format(request('search_price_max'), 0, ',', '.') }}₫"
                                        ...
                                        @if (request('search_status'))
                                            <span class="badge bg-success-subtle text-success">
                                                Trạng thái:
                                                "{{ request('search_status') == 'active' ? 'Hoạt Động' : 'Không Hoạt Động' }}"
                                ...
                                <div class="card-header d-flex justify-content-between align-items-center gap-1">
                                    ...
                                    @else
                                        <a href="{{ route('variants.index', ['show_deleted' => '1']) }}"
                                            ...
                                            @if ($deletedCount > 0)
                                                <span
                                                    class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger animate__animated animate__pulse">
                                                    {{ $deletedCount }}
                                                </span>
                                            @endif
                                        </a>
                                    ...
                                    @if ($variants->count() > 0)
                                                ...
                                                <tbody>
                                                    @foreach ($variants as $variant)
                                                        <tr>
                                                                    ...
                                                                    <div class="flex-grow-1">
                                                                        <h6 class="mb-0 fw-semibold">
                                                                            @if ($variant->product)
                                                                                @if (request('show_deleted') == '1')
                                                                                    <span
                                                                                        class="text-muted">{{ $variant->product->name }}</span>
                                                                                @else
                                                                                ...
                                                                                @endif
                                                                            @else
                                                                                <span class="text-muted">Không tìm thấy sản
                                                                                    phẩm</span>
                                                                            @endif
                                                                        </h6>
                                                            ...
                                                            <td>
                                                                <span
                                                                    class="badge bg-{{ $variant->stock_quantity > 0 ? 'success' : 'danger' }}-subtle 
                                                                        ...
                                                                        @foreach ($variant->attributesValue as $attrValue)
                                                                            @if (request('show_deleted') == '1')
                                                                                <span class="badge bg-light text-muted">
                                                                                    {{ $attrValue->attribute->name ?? 'Thuộc tính đã xóa' }}:
                                                                                    {{ $attrValue->value }}
                                                                                </span>
                                                                            @else
                                                            ...
                                                            <td>
                                                                ...
                                                                @else
                                                                    <div class="form-check form-switch">
                                                                        ...
                                                                        <label class="form-check-label"
                                                                            for="status{{ $variant->id }}">
                                                                            {{ ucfirst($variant->status == 'active' ? 'Hoạt Động' : 'Không Hoạt Động') }}
                                                                        </label>
                                                                    </div>
                                                                @endif
                                                            </td>
                                                                ...
                                                                <div class="dropdown">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end">
                                                                        @if (request('show_deleted') == '1')
                                                                            ...
                                                                            <form
                                                                                action="{{ route('variants.restore', $variant->id) }}"
                                                                                ...
                                                                                <button type="submit"
                                                                                    ...
                                                                                    <i class="bx bx-refresh me-2"></i>Khôi
                                                                                    Phục
                                                                                </button>
                                                                            </form>
                                                                            <form
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-3">
                                            <div class="text-muted">
                                                Hiển thị từ {{ $variants->firstItem() }} đến {{ $variants->lastItem() }}
                                            ...
                                            <div>
                                                <ul class="pagination">
                                                    ...
                                                    @foreach ($variants->getUrlRange(1, $variants->lastPage()) as $page => $url)
                                                        <li
                                                            class="page-item{{ $variants->currentPage() == $page ? ' active' : '' }}">
                                                            <a class="page-link"
                                                                href="{{ $url }}">{{ $page }}</a>
                                                        </li>
                                                    @endforeach
